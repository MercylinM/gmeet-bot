...
if pgrep -x pipewire > /dev/null; then
    ...
    # Wait a bit for PipeWire to fully initialize
    sleep 2
    
    # Create virtual sink and source
    pw-cli create-node adapter factory.name=support.null-audio-sink node.name=virtual_output node.description="Virtual Output" media.class=Audio/Sink audio.position=[FL,FR] || echo "Virtual output already exists or could not be created"
    pw-cli create-node adapter factory.name=support.null-audio-sink node.name=virtual_input node.description="Virtual Input" media.class=Audio/Source audio.position=[FL,FR] || echo "Virtual input already exists or could not be created"
    
    # Set up PulseAudio compatibility layer
    pipewire-pulse &
    PULSE_PID=$!
    ...
    echo "WARNING: PipeWire may not be running properly"
    ps aux | grep pipewire
    echo "Attempting to continue without PipeWire..."
fi

# List available audio devices for debugging
echo "Available audio devices:"
pw-cli list-objects | grep -E "(node.name|node.description)" || echo "Could not list audio devices"
...
# Run the Flask server (bot will wait for HTTP trigger)
echo "Starting Google Meet Bot with PipeWire audio backend..."
# Add error handling to prevent early exit
if python3 gmeet.py; then
    echo "Application exited normally"
else
    echo "Application exited with error code: $?"
    # Don't exit immediately, give time for debugging
    sleep 10
fi
